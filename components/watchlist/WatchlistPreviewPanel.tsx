...
export function WatchlistPreviewPanel({ 
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-black/20 backdrop-blur-sm ${className}`} onClick={onClose}>
      <div 
        className="absolute right-0 top-0 h-full w-[600px] bg-background border-l shadow-2xl transform transition-transform duration-300 ease-out"
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-muted/50 rounded-lg p-4">
                ...
                <p className="text-2xl font-bold">{formatNumber(displayPackage.stars || 0)}</p>
              </div>
              <div className="bg-muted/50 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-2xl font-bold">{formatNumber(displayPackage.downloads || 0)}</p>
              </div>
              
              {/* Additional stats from details view */}
              {displayPackage.forks && (
                <div className="bg-muted/50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <GitFork className="h-5 w-5 text-blue-500" />
                    <span className="font-medium">Forks</span>
                  </div>
                  <p className="text-2xl font-bold">{formatNumber(displayPackage.forks)}</p>
                </div>
              )}
              
              {displayPackage.contributors && (
                <div className="bg-muted/50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Users className="h-5 w-5 text-purple-500" />
                    <span className="font-medium">Contributors</span>
                  </div>
                  <p className="text-2xl font-bold">{formatNumber(displayPackage.contributors)}</p>
                </div>
              )}
            </div>
