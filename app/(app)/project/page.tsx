...
export default function Home() {
    ...
    useEffect(() => {
        ...
        if (typeof window !== "undefined") {
            window.dispatchEvent(new Event("projects:loading"))
        }

        fetchProjects().finally(() => {
            setLoading(false)
            if (typeof window !== "undefined") {
                window.dispatchEvent(new Event("projects:loaded"))
            }
        })
    }, [backendUserId])
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {filteredProjects.map((project) => {
                                    ...
                                    <CardContent className="p-6">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {isCreating && (
                                                    <div className="flex items-center gap-1 text-blue-400 text-xs">
                                                        <div
                                                            className="animate-spin h-3 w-3 border-2 border-blue-400 border-t-transparent rounded-full"></div>
                                                        <span>Creating</span>
                                                    </div>
                                                )}
                                                {isFailed && (
                                        ...
                                        {project.repository_url && project.repository_url.includes('github.com') && (
                                            <div className="flex items-center gap-2 text-gray-400 text-sm mb-3">
                                                ...
                                                <span
                                                    className="truncate">{project.repository_url.split('/').slice(-2).join('/')}</span>
                                            </div>
                                        )}
                            ...
                            {isCreating ? 'Progress' : 'Project Score'}
                                                ...
                                                <span
                                                    className={`text-sm font-medium transition-all duration-1000 ease-out ${
