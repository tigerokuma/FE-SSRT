...
export default function SbomSearchPage() {
  ...
  const smallLicenses = rawData.filter(item => item.value < threshold);

  // Combine small ones into "Other"
  const otherTotal = smallLicenses.reduce((sum, item) => sum + item.value, 0);
  if (otherTotal > 0) {
    bigLicenses.push({ name: "Other", value: otherTotal });
  }
        ...
        <div>
          ...
          <div className="mt-8">
             {history.length > 1 && (
              ...
              >
                Go Back to {history[history.length - 2]}
              </button>
            )}
            <MyGraphComponent 
